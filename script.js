...
function combine(s1, s2, op, value, level, opCount, numbers) {
    ...
    if (op === 'root') {
        ...
        // *** แก้ไข: ใช้รูปแบบ LaTeX/Superscript สำหรับ Power: {base}^{exp} ***
        expr = `{${e1}}^{${e2}}`; 
    } else {
        expr = `${e1} ${op} ${e2}`;
    }
...
function formatSolutionHTML(sol, isClosest = false) {
    ...
    // 1. แปลงสัญลักษณ์รากที่สอง √(...) เป็น \sqrt{...}
    displayExpr = displayExpr.replace(/√\((.*?)\)/g, '\\sqrt{$1}');
    // 2. แปลงสัญลักษณ์รากที่ N (N)√(...) เป็น \sqrt[N]{...}
    displayExpr = displayExpr.replace(/\((\d+)\)√\((.*?)\)/g, '\\sqrt[$1]{$2}');
    
    // 3. จัดการการยกกำลัง: ถูกสร้างในฟังก์ชัน combine ในรูปแบบ {base}^{exp} 
    //    รูปแบบนี้ถูกต้องสำหรับ MathJax อยู่แล้ว
...
function formatSigmaSolution(target, s_sol, e_sol, expr_str) {
    ...
    // เพิ่มวงเล็บครอบ expr_str: (${expr_str})
