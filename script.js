...
function combine(s1, s2, op, value, level, opCount, numbers) {
    ...
    // e1 และ e2 ถูกครอบด้วย () ถ้า prec ต่ำกว่า
    ...
    if (op === 'root') {
        ...
        // *** แก้ไข: ใช้รูปแบบ LaTeX {ฐาน}^{เลขชี้กำลัง} โดยใช้ e1 (ซึ่งมีวงเล็บที่จำเป็นอยู่แล้ว) ***
        expr = `{${e1}}^{${e2}}`; 
    } else {
        expr = `${e1} ${op} ${e2}`;
    }
            ...
            for (const [s_val, s_sol] of starts.entries()) {
                for (const [e_val, e_sol] of ends.entries()) {
                    ...
                    for (const [x_val, x_sol] of exprs.entries()) {
                        ...
                        if (sum === target) {
                             // *** แก้ไข: ใช้ {${x_sol.expr}} เป็นฐานโดยตรง เพื่อให้ได้ (4 - √4)^i ***
