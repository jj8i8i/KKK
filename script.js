...
function applyUnaryOps(resultsMap, level, numbersUsed) {
    ...
    for (const s of currentResults) {
        ...
        if (level >= 3 && v >= 0 && v <= MAX_FACTORIAL && Number.isInteger(v)) {
            ...
            // ถ้า cleanParentheses(s.expr) ไม่เท่ากับ s.expr แสดงว่ามีวงเล็บที่ไม่จำเป็นถูกลบออกไป
            if (s.expr.startsWith('(') && s.expr.endsWith(')')) {
                 // ตรวจสอบเพื่อหลีกเลี่ยงการใส่วงเล็บซ้ำ เช่น ((5!))!
...
function cleanParentheses(s) {
    ...
    while (s.startsWith('(') && s.endsWith(')')) {
        ...
        // ตรวจสอบว่าวงเล็บครอบทั้งหมด (balance = 0)
