...
async function handleSolve() {
    ...
    try {
        ...
        // นี่คือส่วนที่เร็วและต้องทำเสมอ
        ...
        if (level === 3 && timeRemaining > 0) {
            ...
            const sigmaSolutions = await solveSigma(nums, target, deadline);
            allSolutions.push(...sigmaSolutions);
        }
        
    } catch (error) {
        // ในกรณีที่เกิดข้อผิดพลาด (แม้ว่า solveAllSubsets ไม่น่าจะ error)
...
async function solveSubsetRecursive(numbers, level) {
    ...
    if (numbers.length === 1) {
        ...
        await applyUnaryOps(results, level, [n]); // Apply !, sqrt
...
async function solveSigma(numbers, target, deadline) {
    const solutions = [];
    const n = numbers.length;
    ...
    for (let i = 0; i < Math.pow(3, n); i++) {
        // *** TIMEOUT CHECK ***
        // เช็คทุก 100 loops เพื่อประสิทธิภาพ
