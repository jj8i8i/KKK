...
function combine(s1, s2, op, value, level, opCount, numbers) {
    ...
    let expr;
    if (op === 'root') {
        // Nth Root: v2-th root of v1 --> v2 √(v1)
        expr = `(${e2})√(${e1})`; 
    } else {
        expr = `${e1} ${op} ${e2}`;
    }
...
function displaySolutions(solutions, target) {
    ...
    // Sort: 1. diff (น้อยสุด), 2. type (normal ก่อน sigma), 3. level, 4. opCount
    solutions.sort((a, b) => {
    ...
    // *** สั่ง MathJax ให้ทำงานกับผลลัพธ์ใหม่ ***
    if (window.MathJax) {
...
// *** แก้ไขการแสดงผลสำหรับ Sigma และ Root (สร้าง String LaTeX) ***
function formatSolutionHTML(sol, isClosest = false) {
    ...
    // แปลงสัญลักษณ์สำหรับสร้าง String LaTeX
    let displayExpr = sol.expr;
    // แปลงสัญลักษณ์รากที่สอง √(...) เป็น \sqrt{...}
    displayExpr = displayExpr.replace(/√\((.*?)\)/g, '\\sqrt{$1}');
    // แปลงสัญลักษณ์รากที่ N (N)√(...) เป็น \sqrt[N]{...}
    displayExpr = displayExpr.replace(/\((\d+)\)√\((.*?)\)/g, '\\sqrt[$1]{$2}');
    // แปลงสัญลักษณ์ยกกำลัง ^ เป็น \{...\} (ถ้าไม่ได้อยู่ใน Sigma)
    displayExpr = displayExpr.replace(/\^/g, '^{\\small\\text{^}}'); 
...
// *** สร้าง String LaTeX สำหรับ Sigma ***
function formatSigmaSolution(target, s_sol, e_sol, expr_str) {
    // ใช้สัญลักษณ์ LaTeX สำหรับ Sigma
    // **s_sol.expr และ e_sol.expr ถูกแปลงเป็น LaTeX โดย MathJax เองเมื่ออยู่ใน $ $**
    // เราใช้ { } เพื่อครอบเขตของตัวห้อยและตัวยกของ Sigma
