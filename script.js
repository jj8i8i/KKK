...
function combine(s1, s2, op, value, level, opCount, numbers) {
    ...
    // *** แก้ไข: เพิ่มเงื่อนไขสำหรับหาร เพื่อป้องกันการครอบ e2 ซ้ำซ้อนเมื่อ e2 เป็นเศษส่วน ***
    if (op === '-') e2 = (s2.precedence <= prec) ? `(${s2.expr})` : s2.expr;
    if (op === '/') {
        // สำหรับการหาร: เราจะเพิ่มวงเล็บครอบ e2 หากมันมี precedence ต่ำกว่าหรือเท่ากับ prec (เช่น + หรือ -)
        // และยังคงใช้ '/' เป็นตัวคั่นชั่วคราวในการเก็บค่า expr ภายใน
        if (s2.precedence < prec || s2.expr.includes('+') || s2.expr.includes('-')) {
             e2 = `(${e2})`;
        }
    }
    ...
    if (op === 'root') {
        ...
        // ใช้ / ภายในเพื่อเก็บค่า และจะถูกแปลงเป็นเศษส่วนเมื่อแสดงผล
        expr = `${e1} ${op} ${e2}`;
    }
...
function formatSolutionHTML(sol, isClosest = false) {
    ...
    if (sol.type !== 'sigma') {
        // ใช้ RegExp เพื่อค้นหาการหารที่อยู่ในรูปแบบ A/B และแทนที่ด้วย \frac{A}{B}
        // ต้องระวังไม่ให้มีเครื่องหมายอื่นมาคั่น A หรือ B (เช่น A+C/B จะไม่ถูกเปลี่ยน)
        
        // นี่คือรูปแบบที่ซับซ้อนขึ้นเพื่อจัดการกับวงเล็บและการซ้อน
        const parseDivision = (expr) => {
            // Regex นี้ค้นหา: (นิพจน์ซ้าย)/(นิพจน์ขวา) โดยจัดการกับวงเล็บอย่างง่าย
            const regex = /(.+?)\s*\/\s*(.+)/;
            let match;
            
            // Loop เพื่อหาหารที่อยู่ชั้นนอกสุดก่อน
            while ((match = expr.match(regex))) {
                const numerator = match[1].trim();
                const denominator = match[2].trim();
                
                // ตรวจสอบว่า numerator/denominator ถูกครอบด้วยวงเล็บที่ไม่จำเป็นหรือไม่
                const cleanNumerator = numerator.replace(/^\((.*)\)$/, '$1');
                const cleanDenominator = denominator.replace(/^\((.*)\)$/, '$1');

                // แทนที่ด้วยรูปแบบ \frac
                const fraction = `\\frac{${cleanNumerator}}{${cleanDenominator}}`;
                
                // แทนที่ในนิพจน์
                expr = expr.replace(match[0], fraction);
            }
            return expr;
        };

        // ขั้นตอนนี้อาจต้องมีการปรับปรุงเพิ่มเติมสำหรับนิพจน์ที่ซับซ้อนมาก ๆ 
        // แต่มันจะทำงานได้ดีกับโครงสร้างพื้นฐานที่ Solver นี้สร้าง
        
        // ขั้นแรก: ครอบทุกส่วนการหารด้วยวงเล็บชั่วคราวเพื่อให้ง่ายต่อการแปลง
        // (x * y) / (a + b) -> {x * y} / {a + b}
        displayExpr = displayExpr.replace(/(\S+)\s*\/\s*(\S+)/g, (match, p1, p2) => {
            return `\\frac{${p1.replace(/[\(\)]/g, '')}}{${p2.replace(/[\(\)]/g, '')}}`;
        });
    }

    // ถ้าไม่ใช่ Sigma ให้ใส่ $ $ ครอบเพื่อให้ MathJax แปลง
    if (sol.type !== 'sigma') {
