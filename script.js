...
function cleanParentheses(s) {
    s = s.trim();
    // เพิ่มเงื่อนไขให้ไม่ล้างวงเล็บถ้าเป็น (A + B) หรือ (A - B) โดยมี * หรือ / อยู่นอกวงเล็บ
    // หรือถ้าเป็นวงเล็บของฟังก์ชันเช่น (\sqrt{...})
    let originalS = s;
    let iterations = 0;
    while (s.startsWith('(') && s.endsWith(')')) {
        // จำกัดจำนวนรอบเพื่อป้องกัน infinite loop (ถ้ามีปัญหา)
        iterations++;
        ...
        if (isFullyCovered && balance === 0) {
            ...
            // ตรวจสอบเงื่อนไขเพิ่มเติมก่อนลบวงเล็บ:
            // 1. ถ้าภายในมีเครื่องหมาย + หรือ - และมีตัวดำเนินการอื่นอยู่ข้างนอกวงเล็บ (แสดงว่าวงเล็บจำเป็น)
            // 2. ถ้าเป็นแค่ตัวเลขเดี่ยวๆ หรือมีแค่ * / ^, สามารถลบวงเล็บได้
            // 3. ถ้าเป็นพวก factorial, root, power, ให้คงวงเล็บไว้
            
            // ตัวอย่าง: (A + B) * C => ควรคงวงเล็บ (A+B) ไว้
            // (A) * C => สามารถลบวงเล็บได้
            
            // Regex เพื่อตรวจหา + หรือ - ที่ไม่ได้อยู่ในวงเล็บย่อย
            const hasPlusMinus = /[+-]/.test(innerContent);
            
            if (hasPlusMinus) {
                // ถ้ามี +/- ข้างใน ให้คงวงเล็บไว้ เพื่อให้ precedence ถูกต้อง
                break; 
            }
            
            // ถ้าไม่มี +/- ข้างใน หรือมีแต่ * / ^
            s = innerContent;
        } else {
            break;
        }
...
function formatSolutionHTML(sol, isClosest = false) {
    ...
    displayExpr = displayExpr.replace(/√\[(.*?)\]/g, (match, content) => {
        ...
        const cleanedContent = cleanParentheses(content); // ใช้ cleanParentheses ที่ปรับปรุงแล้ว
    ...
    displayExpr = cleanParentheses(displayExpr); // ใช้ cleanParentheses ที่ปรับปรุงแล้ว
...
function processMultiplicationDivision(expr) {
    ...
    while (tempExpr.includes('/')) {
        ...
        let denominator = cleanParentheses(denominatorPart); // ใช้ cleanParentheses ที่ปรับปรุงแล้ว
        ...
        for (let k = numeratorPart.length - 1; k >= 0; k--) {
            ...
            } else if (balance === 0 && char === '+') { // ตรวจสอบ '+' ที่อาจเหลืออยู่ใน currentPart (ควรถูกแยกไปตั้งแต่ต้นแล้ว)
                 // อันนี้ไม่ควรเกิดขึ้นถ้า formatSolutionHTML ทำงานถูก
                 lastOpIndex = k;
                 break;
            }
        }
        ...
        if (lastOpIndex !== -1) {
            ...
            numerator = cleanParentheses(numeratorPart.substring(lastOpIndex + 1).trim()); // ใช้ cleanParentheses ที่ปรับปรุงแล้ว
            remainingPart = numeratorPart.substring(0, lastOpIndex + 1).trim();
            
            tempExpr = `${remainingPart} \\frac{${numerator}}{${denominator}}`;
        } else {
            // A คือทั้งก้อน
            numerator = cleanParentheses(numeratorPart); // ใช้ cleanParentheses ที่ปรับปรุงแล้ว
...
function formatSigmaSolution(target, s_sol, e_sol, expr_str) {
    ...
    // ** การแก้ไข: ใส่วงเล็บครอบ expression ของ Sigma หากจำเป็น **
    // ตรวจสอบว่า expression มีเครื่องหมาย + หรือ - ที่ไม่ได้อยู่ในวงเล็บย่อย
    // หรือถ้าเป็นพวกตัวเลขเดี่ยว ๆ ก็ไม่จำเป็นต้องมีวงเล็บ
    const requiresParentheses = /[+-]/.test(expr_str) || expr_str.includes('\\times') || expr_str.includes('\\frac');
    
    let final_expr_str = cleanParentheses(expr_str); // ลองล้างวงเล็บที่ไม่จำเป็นออกก่อน

    // หากหลังจากล้างแล้วยังต้องใส่วงเล็บ (เช่น มี +,- หรือเป็นเศษส่วน/คูณ) ให้ใส่วงเล็บครอบ
    if (requiresParentheses && !final_expr_str.startsWith('(') && !final_expr_str.startsWith('\\(') && !final_expr_str.startsWith('{')) {
