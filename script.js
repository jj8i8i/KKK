...
function combine(s1, s2, op, value, level, opCount, numbers) {
    ...
    if (op === '/') {
         // สำหรับหาร: จะจัดการวงเล็บในการแสดงผล HTML แทน เพื่อความแม่นยำในการครอบคลุม
         // ในที่นี้ยังคงเก็บค่า e1/e2 ไว้ในรูปแบบ string เหมือนเดิม
    }
...
function formatSolutionHTML(sol, isClosest = false) {
    ...
    if (sol.type !== 'sigma') {
        // *** Logic การแปลงเศษส่วนที่ฉลาดขึ้น ***
        
        // ฟังก์ชันช่วยล้างวงเล็บครอบนอกสุดที่ไม่จำเป็น
        const cleanParentheses = (s) => {
            s = s.trim();
            while (s.startsWith('(') && s.endsWith(')')) {
                // ตรวจสอบว่าวงเล็บนอกสุดครอบคลุมทั้งหมดหรือไม่
                let balance = 0;
        ...
        // ใช้ Regex เพื่อจับคู่ A / B โดย A หรือ B อาจเป็นนิพจน์ที่ซับซ้อน (ใช้วงเล็บ)
        // Note: Regex ใน JS มีข้อจำกัดในการจับคู่วงเล็บซ้อนที่ซับซ้อนมาก ๆ 
        // แต่เราจะใช้การจับคู่แบบ non-greedy และสมมติว่านิพจน์ที่สร้างโดย combine() มีโครงสร้างค่อนข้างดี

        // ตัวอย่างการทำงาน: (6!) / (3 + 3 + 3)
        // A: 6!, B: 3 + 3 + 3
        
        // Step 1: แยกนิพจน์ทั้งหมดออกจาก +, - ที่อยู่นอกสุด (เพราะ +,- มี precedence ต่ำสุด)
        let parts = displayExpr.split(/([+\-])/g).map(p => p.trim()).filter(p => p.length > 0);
        let finalExpr = '';
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            // Step 2: ค้นหาการหารภายในแต่ละส่วนย่อย (เช่น 6!/(3+3+3) * 2)
            // ใช้การจับคู่ที่ไม่ซับซ้อนเกินไป (เน้นเครื่องหมายคูณ/หาร)
            
            // Regex: A / B โดย A/B อาจมี * หรือ / ได้ แต่ไม่ควรมี + หรือ - (เพราะถูกแยกไปแล้ว)
            // หรือ A/B เป็นวงเล็บครอบนิพจน์ (X)/Y
            
            // ใช้การวนซ้ำเพื่อเปลี่ยนหาร (/) เป็น \frac{A}{B} จากซ้ายไปขวา
            while (part.includes('/')) {
                // ค้นหาสูตร A / B
                const divisionRegex = /(.+?)\s*\/\s*(.+)/;
                const match = part.match(divisionRegex);
                
                if (!match) break; // ไม่เจอการหารแล้ว
                
                let numerator = match[1].trim();
                let denominator = match[2].trim();
                
                // ตัวอย่าง: ถ้าเป็น 6! / (3+3+3)
                // numerator = 6!
                // denominator = (3+3+3)

                // ล้างวงเล็บที่ไม่จำเป็นออกจากการแสดงผล
                numerator = cleanParentheses(numerator);
                denominator = cleanParentheses(denominator);
                
                // แทนที่นิพจน์หารทั้งหมดด้วย \frac{A}{B}
                const fraction = `\\frac{${numerator}}{${denominator}}`;
                part = part.replace(match[0], fraction);
            }
