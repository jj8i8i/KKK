...
function handleSolve() {
    ...
    // สร้าง copy ของ nums array เพื่อป้องกันการแก้ไขโดยไม่ตั้งใจ
    ...
    // *** นี่คือส่วนสำคัญ ***
    // เราใช้ setTimeout(0) เพื่อให้เบราว์เซอร์แสดง "loadingSpinner"
    // ก่อนที่จะเริ่มการคำนวณหนัก (ซึ่งจะ block thread)
    setTimeout(() => {
        let allSolutions = [];

        try {
            // 3. Run the Dynamic Programming Solver (Non-Sigma)
            // นี่คือส่วนที่เร็วและต้องทำเสมอ (รันแบบ Synchronous)
            ...
            if (level === 3 && timeRemaining > 0) {
                ...
                // 'originalNums' ถูก sort ไปแล้ว ไม่เป็นไร
...
function solveSubsetRecursive(numbers, level) {
    ...
    if (numbers.length === 1) {
        ...
        applyUnaryOps(results, level, [n]); // Apply !, sqrt
    ...
    for (let i = 1; i < (1 << n) / 2; i++) {
        ...
        for (const [v1, s1] of results1.entries()) {
            for (const [v2, s2] of results2.entries()) {
                ...
                // Nth Root (Lv.2)
                if (level >= 2) {
                    if (v1 > 0 && v2 > 1 && v2 < 10) { // v2-th root of v1
                    ...
                    if (v2 > 0 && v1 > 1 && v1 < 10) { // v1-th root of v2
    ...
    // Unary Ops (sqrt, !) on combined results (e.g., sqrt(4+8))
...
function addSolution(map, newSol) {
    if (newSol.value > 99999 || newSol.value < 0) return; // Pruning
    ...
    if (newSol.level < existingSol.level) {
        ...
        map.set(newSol.value, newSol); // Shorter expression is simpler
    }
}
            ...
            for (const [s_val, s_sol] of starts.entries()) {
                for (const [e_val, e_sol] of ends.entries()) {
                    ...
                    for (const [x_val, x_sol] of exprs.entries()) {
                        ...
                        if (sum === target) solutions.push(formatSigmaSolution(target, s_sol, e_sol, `(${x_sol.expr})^i - i`));
                        
                        // Test Template: (X^i)
                        sum = 0;
                        for (let k = s_val; k <= e_val; k++) sum += Math.pow(x_val, k);
                        if (sum === target) solutions.push(formatSigmaSolution(target, s_sol, e_sol, `(${x_sol.expr})^i`));
                    }
                }
            }
        }
...
function displaySolutions(solutions, target) {
    ...
    // Sort: 1. diff (น้อยสุด)
    //       2. type (sigma มาทีหลัง ถ้า diff เท่ากัน)
    //       3. level (น้อยสุด - นี่คือ "ง่ายที่สุด")
    //       4. opCount (น้อยสุด - นี่คือ "ง่ายที่สุด")
    solutions.sort((a, b) => {
        if (a.diff !== b.diff) return a.diff - b.diff;
        if (a.type !== 'sigma' && b.type === 'sigma') return -1; // ปกติมาก่อน
...
function formatSigmaSolution(target, s_sol, e_sol, expr_str) {
    ...
    return {
        value: target,
        expr: expr,
        opCount: s_sol.opCount + e_sol.opCount + 99, // Sigma ถือว่าซับซ้อน
        level: 3,
