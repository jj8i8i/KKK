...
// *** ฟังก์ชันช่วยล้างวงเล็บครอบนอกสุดที่ไม่จำเป็น (ปรับปรุงให้ชาญฉลาดขึ้น) ***
function cleanParentheses(s) {
    ...
    while (s.startsWith('(') && s.endsWith(')')) {
        ...
        // ตรวจสอบว่าวงเล็บครอบทั้งหมด (balance = 0) และไม่มีวงเล็บซ้อน
...
function processMultiplicationDivision(expr) {
    ...
    // การแปลงเศษส่วนทำงานจากขวาไปซ้าย (A / B / C = A / (B / C)) 
    // เพื่อให้ได้ A / B / C
    ...
    while (tempExpr.includes('/')) {
        ...
        // เราจะเอาวงเล็บครอบนอกสุดที่ไม่จำเป็นออก
        let denominator = cleanParentheses(denominatorPart);

        // 2. ตัวเศษ (Numerator): A คือนิพจน์ที่อยู่หน้า /
        // A ต้องครอบคลุมนิพจน์ทั้งหมดที่อยู่หน้า / จนกว่าจะเจอเครื่องหมาย +,-,* ที่อยู่นอกวงเล็บที่กำลังพิจารณา
        
        let lastOpIndex = -1;
        let balance = 0;
        // ไล่จากขวาไปซ้ายใน numeratorPart เพื่อหาวงเล็บหรือตัวดำเนินการ * ที่จะหยุด A
        // ถ้าเป็น (A+B)*C / D, A คือ (A+B)*C
        // ถ้าเป็น A * B / C, A คือ A * B
        ...
        for (let k = numeratorPart.length - 1; k >= 0; k--) {
            ...
            // หยุดเมื่อเจอเครื่องหมายคูณ (*) นอกวงเล็บ
            if (balance === 0 && char === '*') {
                 lastOpIndex = k;
                 break;
            }
